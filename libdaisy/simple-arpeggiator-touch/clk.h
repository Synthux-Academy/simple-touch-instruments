...
inline static bool fcomp(const float lhs, const float rhs,
...
template <size_t ppqn> class Clock {
public:
  Clock()
      : _on_tick{nullptr}, _is_running{false}, _is_about_to_run{false},
        _tr_time{0}, _ticks_per_clock{ppqn / 4}, _ticks{0}, _fticks{0},
        _ticks_at_last_clock{0}, _tempo_ticks{0}, _hold{false}, _resync{false},
        _manual_tempo{120}, _raw_manual_tempo{120}, _tempo_mks{500000},
        _last_state{1} {}
  ...
  void SetOnTick(void (*on_tick)()) { _on_tick = on_tick; }
  ...
  */
  void SetTempo(float norm_value) {
    if (fcomp(norm_value, _raw_manual_tempo))
      return;
    _raw_manual_tempo = norm_value;
    const auto clock_off_offset = 10;
    _manual_tempo = (kBPMRange - clock_off_offset) * norm_value + kBPMMin -
                    clock_off_offset;
...
private:
  ...
  */
  void external_clock_tick() {
    if (!external_clock())
      return;
    if (!_is_running && !_is_about_to_run)
      return;
  ...
  /*
  Derived from Maximum MIDI Programmer's ToolKit Copyright ©1993-1998 by Paul
  Messick. This method generates internal ticks and also synchronises to the
  external clock. So it's called both from internal interrupt timer (audio
  callback in this implementation) and upon external clock tick reception.
  ...
  _resync - flag to resync to external clock. Is set to true once external clock
  tick is received. _hold - flag to stop advancing internal timeline if the
  number of internal ticks exceeded expected count of internal ticks per extrnal
  tick _tr_time - internal resolution (ppqn) multiplied by interrupt interval.
  */
  void emit_ticks() {
    uint32_t nticks = 0;
    ...
    if (_resync) {
      ...
      _tempo_mks -= ((int32_t)_ticks_per_clock - (int32_t)_tempo_ticks) *
                    (int32_t)_tempo_mks / (int32_t)ppqn;
      _tempo_ticks = 0;
      _resync = false;
    }
    // Regular mode. We generate internal ticks.
      ...
      if (external_clock()) {
        ...
        // expected, we set _hold to true effectively stopping advancing
        // timeline until next external tick
