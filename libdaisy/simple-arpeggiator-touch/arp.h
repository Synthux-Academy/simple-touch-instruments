...
enum class ArpDirection { fwd, rev };

template <uint8_t note_count, uint8_t ppqn> class Arp {
public:
  Arp()
      : _on_note_on{nullptr}, _on_note_off{nullptr},
        _direction{ArpDirection::fwd}, _rand_chance{0}, _played_idx{0},
        _as_played{false}, _note_length{ppqn / 12}, _max_note_length{ppqn / 4 +
                                                                     1},
        _bottom_idx{0}, _current_idx{0}, _pulse_counter{0}, _size{0} {
    _SetMaxNoteLength();
    Clear();
  }
  ...
  void SetDirection(ArpDirection direction) { _direction = direction; }

  void SetRandChance(float rand) { _rand_chance = rand; }

  void SetAsPlayed(bool value) { _as_played = value; }

  void SetNoteLength(float length) { _note_length = _max_note_length * length; }
  ...
  void Trigger() {
    ...
    if (_note_length < ppqn / 4 && _pulse_counter == _note_length &&
        _current_idx > 0) {
      _on_note_off(_notes[_current_idx].num);
    }

    // We trigger on every 1/16th
    if (_pulse_counter++ < ppqn / 4)
      return;
    _pulse_counter = 0;
    ...
    if (_rand_chance > 0.05 && _rand_chance < 0.95) {
      auto rnd = static_cast<float>(_rnd.generate(0, 100)) / 100.f;
      if (rnd <= _rand_chance) {
        note_idx = _input_order[_rnd.generate(0, _size - 2)];
      }
    }
